...
- Train/eval split: smoke training and toy validation both generate from the same deterministic rule; this is not an independent corpus split.
...
- Limitations: very low entropy, no semantics, no realistic language structure.
- Why this is not a real language benchmark: the model can solve it by memorizing a short periodic transition table.
- Future replacement: a governed tokenizer and held-out text or structured benchmark suite with documented licenses, splits, deduplication, and contamination checks.

## `toy/copy`

- Dataset name: `toy/copy`
- Purpose: test short exact copying under a deterministic generated format.
- Generation method: sample seeded token payloads, insert delimiter-like high token IDs, and repeat the payload.
- Train/eval split: generated by seed and split offset; validation uses a different deterministic seed stream.
- Expected behavior: models with enough context and training should learn short exact copying under the toy format.
