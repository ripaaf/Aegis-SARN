# Model Card: SARN-Dense Phase 1-7

## Overview

- Model name: SARN-Dense
- Current role: dense Transformer control baseline for Aegis-SARN
- Phase covered: Phase 1 hardened baseline through the Phase 7 working-memory prototype
...
The implementation intentionally excludes a full SARN-Hybrid path, persistent memory, MoE, SSM/Mamba, retrieval, tools, VLM, SAM, LAM, multimodal modules, and advanced safety systems.
...
Phase 1-7 training and evaluation use generated toy token batches. These datasets are deterministic fixtures, not real corpora.

## Evaluation Data

Phase 2-7 evaluation uses generated toy validation batches and deterministic task variants. Phase 6 adds relation-chain, route-propagation, slot-binding, and tiny length-extrapolation fixtures. Phase 7 adds key/value recall, distractor recall, reset isolation, overwrite conflict, tiny capacity stress, and delayed copy. The metrics are useful for regression testing, controlled comparisons, and reproducibility checks only. They do not measure natural-language quality, establish reasoning, or demonstrate useful memory.
...
SARN-Dense remains the control baseline, MHA remains its default attention setting, and workspace, graph, and memory modules remain disabled by default. Experimental GQA, latent slots, graph cycles, and temporary memory are research configurations, not a SARN-Hybrid model path. Future work must compare against the control with matched metrics, seeds, artifacts, and system measurements.
