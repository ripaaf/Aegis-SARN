...
This roadmap is ordered by dependency and evidence, not by how advanced a technique sounds. Phases 1–8 build and qualify the SARN-Dense control, evaluation laboratory, efficient-attention foundation, feature-flagged latent workspace, bounded graph and working-memory prototypes, and sparse expert-routing prototype. Phase 9 tests the later proposed hybrid sequence engine; Phases 10–16 add governed system capabilities; Phases 17–18 are integration gates. Framework and model tracks can advance in parallel after their shared contracts exist. A phase may end in a documented rejection; rejected mechanisms are replaced by their controls rather than forced into the integrated architecture.
...
- **Phase 8 sparse expert-routing prototype:** implemented behind disabled-by-default SARN-Dense configuration. It provides selected-layer top-k local FFN routing, dense/null/two-expert/four-expert controls, optional shared experts, active/total parameter accounting, routing/load diagnostics, cached-generation parity, sweep/comparison reports, and correctness-oriented gates. It has no distributed expert parallelism, custom kernel, or token dropping.
- **Acceptance workflow:** the documented CPU path installs, tests, reproduces earlier phases, sweeps matched dense, MHA/GQA, workspace, graph, memory, and expert variants, benchmarks generation and resource cost, records registry entries, compares results, checks correctness and quality gates, and generates reports while preserving structured metrics and traces.
- **Later phases:** Phase 9 hybrid sequence-engine research is next/planned. No full SARN-Hybrid model path, SSM/Mamba, persistent memory, retrieval, tools, VLM, SAM, LAM, advanced safety system, or multimodal implementation has started.

This progress note records the verified Phase 1-8 SARN-Dense research contract. Phase 8 adds a tiny sparse FFN routing experiment and a matched evidence path inside SARN-Dense; it does not establish expert specialization, production/distributed MoE, self-improvement, SSM/Mamba, or SARN-Hybrid. Implementing the prototype and its correctness gates does not accept it into a final architecture. All later model and system mechanisms remain proposals until their evidence gates pass.
...
**Current implementation:** Phase 8 provides a one-process PyTorch reference router for selected SARN-Dense FFNs. It activates top-k local experts per token and measures load balance, normalized entropy, assignment extremes, and active/total parameters. Dense and exact null controls plus two/four-expert variants, checkpoints, cached generation, sweeps, reports, and correctness gates are implemented. Capacity factor is recorded but no tokens are dropped. These tiny toy results do not satisfy the full workstream exit gate, demonstrate specialization, or establish distributed/full MoE scaling.
