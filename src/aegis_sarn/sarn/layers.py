...
class RotaryEmbedding(nn.Module):
    ...
    def forward(self, query: Tensor, key: Tensor) -> tuple[Tensor, Tensor]:
        ...
        if sequence_length > self.cos_cache.shape[0]:
            raise ValueError('sequence length exceeds the configured RoPE cache')
        cos = self.cos_cache[:sequence_length].to(device=query.device, dtype=query.dtype)
        sin = self.sin_cache[:sequence_length].to(device=query.device, dtype=query.dtype)
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, inputs: Tensor) -> Tensor:
        ...
        future_mask = torch.ones(
            sequence_length, sequence_length, device=inputs.device, dtype=torch.bool
        ).triu(diagonal=1)
