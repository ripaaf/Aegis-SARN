...
def test_manifests_include_required_phase3_fields(
    ...
    for manifest_path in summary['results'][0]['manifest_paths'].values():
        manifest = json.loads(Path(manifest_path).read_text(encoding='utf-8'))
        assert required.issubset(manifest)


def test_check_gates_rejects_non_finite_metric_and_missing_artifact(
    capsys: object, tmp_path: Path
) -> None:
    sweep_dir = _run_tiny_sweep(capsys, tmp_path)
    summary_path = sweep_dir / 'sweep-summary.json'
    summary = json.loads(summary_path.read_text(encoding='utf-8'))
    summary['results'][0]['eval_loss'] = float('nan')
    summary['results'][0]['checkpoint_path'] = str(tmp_path / 'missing.pt')
    invalid_path = tmp_path / 'invalid-summary.json'
    invalid_path.write_text(json.dumps(summary), encoding='utf-8')

    result = check_gates(invalid_path)
    failed_names = {check['name'] for check in result['checks'] if not check['passed']}

    assert result['passed'] is False
    assert 'micro:eval_loss_finite' in failed_names
    assert 'micro:checkpoint_exists' in failed_names
...
def test_no_phase9_or_later_source_modules_are_required() -> None:
